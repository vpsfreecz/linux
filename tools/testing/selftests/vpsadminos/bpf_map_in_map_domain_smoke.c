...
static int consumer_main(struct child_cfg *cfg)
{
	...
	send_errno = send_fd(cfg->sockfd, outer_fd);
	dprintf(cfg->outfd, "consumer_send_outer_errno=%d\n", send_errno);
	send_errno = send_fd(cfg->sockfd, inner_fd);
	dprintf(cfg->outfd, "consumer_send_inner_errno=%d\n", send_errno);

	recv_errno = recv_fd(cfg->sockfd, &cross_fd);
	dprintf(cfg->outfd, "child_cross_domain_recv_errno=%d\n", recv_errno);
	if (!recv_errno) {
		err = update_outer_map(outer_fd, cross_fd);
		dprintf(cfg->outfd, "child_cross_domain_errno=%d\n", err);
		close(cross_fd);
	}
...
static int child_main(void *arg)
{
	...
	close(cfg->parent_startfd);
	close(cfg->parent_outfd);
	close(cfg->parent_sockfd);
	if (cfg->inherited_closefd >= 0)
		close(cfg->inherited_closefd);
...
static int cleanup_failed_spawn(struct domain_child *child, int err)
{
	...
	if (child->pid > 0) {
		...
		do {
			wait_ret = waitpid(child->pid, NULL, 0);
...
static int spawn_domain_child(enum child_mode mode, const char *name,
			      int tokenless_fd, int inherited_closefd,
			      struct domain_child *child)
{
	...
	cfg->parent_startfd = start_pipe[1];
	cfg->parent_outfd = out_pipe[0];
	cfg->parent_sockfd = sock_pair[0];
	cfg->inherited_closefd = inherited_closefd;
...
int main(void)
{
	...
	int tokenless_fd, cross_fd = -1, outer_fd = -1, same_fd = -1;
	int err, child_rc, recv_errno, recv_same_errno, send_errno;
	int output_errno;
	...
	err = spawn_domain_child(CHILD_PRODUCER, name_b, -1, tokenless_fd,
				 &producer);
	if (err) {
	...
	err = spawn_domain_child(CHILD_CONSUMER, name_a, tokenless_fd,
				 cross_fd, &consumer);
	if (err) {
	...
	send_errno = send_fd(consumer.sockfd, cross_fd);
	dprintf(STDOUT_FILENO, "consumer_send_fd_errno=%d\n", send_errno);
	recv_errno = recv_fd(consumer.sockfd, &outer_fd);
	dprintf(STDOUT_FILENO, "host_outer_recv_errno=%d\n", recv_errno);
	recv_same_errno = recv_fd(consumer.sockfd, &same_fd);
	dprintf(STDOUT_FILENO, "host_inner_recv_errno=%d\n", recv_same_errno);
	close(consumer.sockfd);

	output_errno = forward_child_output(consumer.outfd);
	close(consumer.outfd);
	child_rc = wait_domain_child(&consumer);

	if (!send_errno && !recv_errno && !recv_same_errno &&
	    !output_errno && !child_rc) {
		err = update_outer_map(outer_fd, same_fd);
		dprintf(STDOUT_FILENO, "host_same_domain_errno=%d\n", err);
		err = update_outer_map(outer_fd, cross_fd);
		dprintf(STDOUT_FILENO, "host_cross_domain_errno=%d\n", err);
	}
	...
	close(cross_fd);
	close(tokenless_fd);
	return send_errno || recv_errno || recv_same_errno ||
	       output_errno || child_rc ? 1 : 0;
}
