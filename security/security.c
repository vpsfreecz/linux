...
int security_ptrace_access_check(struct task_struct *child, unsigned int mode)
{
	...
	result = security_task_pair_try_begin_where(child, &snapshot, __func__);
	if (result == AUTH_GUARD_CHECK_BUSY)
		return -EAGAIN;
	if (result != AUTH_GUARD_CHECK_VALID)
		return rc;

	rc = call_int_hook(ptrace_access_check, child, mode);
	if (!security_task_snapshots_end_where(&snapshot, __func__))
		rc = -EACCES;
	return rc;
}
...
{
	BUG_ON(!auth_guard_task_check_in_transition(current));
			    ...
			    struct rlimit *new_rlim)
{
	...
	result = security_task_pair_try_begin_where(p, &snapshot, __func__);
	if (result == AUTH_GUARD_CHECK_BUSY)
		return -EAGAIN;
	if (result != AUTH_GUARD_CHECK_VALID)
		return rc;

	rc = call_int_hook(task_setrlimit, p, resource, new_rlim);
	if (!security_task_snapshots_end_where(&snapshot, __func__))
		rc = -EACCES;
	return rc;
}
...
int security_task_setscheduler(struct task_struct *p)
{
	...
	rc = call_int_hook(task_setscheduler, p);
	if (!security_task_snapshots_end_where(&snapshot, __func__))
		rc = -EACCES;
	return rc;
}
...
/**
 * security_task_setscheduler_unpublished() - Check an unpublished fork child
 * @p: unpublished child task
 *
 * Check scheduler permissions before cgroup fork finalization. The private
 * child has no task seal yet, so validate its constructor state while holding
 * the current task's normal reader reservation.
 *
 * Return: Returns 0 if permission is granted.
 */
int security_task_setscheduler_unpublished(struct task_struct *p)
{
	...
	if (auth_guard_task_snapshot_begin(current) != AUTH_GUARD_CHECK_VALID)
		return rc;
	snapshot.current_held = true;
	if (!auth_guard_task_unpublished(p) || !auth_guard_task_init_check(p))
		goto out;

	/* Capability needs the constructor credential accessor; retain all others. */
	rc = cap_task_setscheduler_unpublished(p);
	if (rc)
		goto out;
	lsm_for_each_hook(scall, task_setscheduler) {
		if (scall->hl->lsmid->id == LSM_ID_CAPABILITY)
			continue;
		rc = scall->hl->hook.task_setscheduler(p);
		if (rc != LSM_RET_DEFAULT(task_setscheduler))
			break;
	}
	if (!auth_guard_task_unpublished(p) || !auth_guard_task_init_check(p))
		rc = -EACCES;
