 ...
 */
static int cap_safe_nice_cred(const struct cred *pcred)
{
	int is_subset, ret = 0;
...
static int cap_safe_nice(struct task_struct *p)
{
	...
	return cap_safe_nice_cred(pcred);
}
...
/**
 * cap_task_setscheduler_unpublished - Check a private fork child
 * @p: unpublished child task
 *
 * Apply the scheduler capability decision to the child's exact authenticated
 * constructor credentials without treating the child as a sealed live task.
 *
 * Return: 0 if permission is granted, -ve if denied.
 */
int cap_task_setscheduler_unpublished(struct task_struct *p)
{
	const struct cred *pcred __free(put_cred) =
		get_task_cred_unpublished_checked(p);
