...
#ifdef CONFIG_SECURITY_SMACK_BRINGUP
static int smk_bu_task_known(struct task_struct *otp,
			     struct smack_known *smk_task, int mode, int rc)
{
	struct task_smack *tsp = smack_cred(current_cred());
	char acc[SMK_NUM_ACCESS_TYPE + 1];
...
{
	return smk_bu_task_known(otp, smk_of_task_struct_obj(otp), mode, rc);
}
#else
static inline int smk_bu_task_known(struct task_struct *otp,
...
static int smack_task_setscheduler(struct task_struct *p)
{
	const struct cred *cred __free(put_cred) =
		get_task_cred_checked_nowait(p);
	struct smk_audit_info ad;
	struct smack_known *skp;
	int rc;

	if (PTR_ERR_OR_ZERO(cred) == -EACCES)
		cred = get_task_cred_unpublished_checked(p);
	if (IS_ERR(cred))
		return PTR_ERR(cred);
	...
	smk_ad_init(&ad, __func__, LSM_AUDIT_DATA_TASK);
	smk_ad_setfield_u_tsk(&ad, p);
	rc = smk_curacc(skp, MAY_WRITE, &ad);
	rc = smk_bu_task_known(p, skp, MAY_WRITE, rc);
	return rc;
}
