...
#define	MEM_CGROUP_MAX_KSOFTLIMD_RECLAIM_LOOPS	32
...
static int ksoftlimd(void *p)
{
	...
	for ( ; ; ) {
		unsigned long soft_limit = memcg->soft_limit;
		...
		remaining = schedule_timeout(HZ*sleep_msec/1000);
		...
		if (remaining)
			/*
			 * We were woken up prematurely, that most likely means
			 * we're tight on memory, go really fast.
			 */
			sleep_msec = 10;
		else if ((scanned == 0) && !remaining)
			/*
			 * We've scanned nothing and didn't up prematurely,
			 * we should slow down a little.
			 */
			sleep_msec += 100;
		else if (reclaimed >= excess)
			/*
			 * We've scanned and reclaimed enough, next time sleep
			 * longer.
			 */
			sleep_msec += 1000;
		else if ((excess > (soft_limit*90/100)) && (sleep_msec > 2000))
			/*
			 * Once we get to 90% of soft_limit, we accelerate
			 * the reclaim loop to get faster to about 1 second.
			 */
			sleep_msec -= 1000;
		else if (excess > (soft_limit*10/100))
			/*
			 * In ideal case, if it is possible, we try to hover
			 * around soft_limit. If we start slipping, go faster.
			 */
			sleep_msec -= 100;
		/*
		 * The sleep_msec adjustment above can cause it to go out of our
		 * desired time bounds, so enforce them here.
		 */
		if (sleep_msec < 10)
			sleep_msec = 10;
		if (sleep_msec > 15000)
		...
		pr_debug("ksoftlimd run for memcg %d sleep_msec %li scanned %lu reclaimed %lu remaining %li\n",
		    memcg->css.id, sleep_msec, scanned, reclaimed, remaining);
