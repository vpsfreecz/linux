...
#define get_task_cred_unpublished_checked(_task) \
	get_task_cred_unpublished_checked_where((_task), __func__)
#define prepare_creds_setid() prepare_creds_setid_where(__func__)
#define commit_creds(_new) commit_creds_where((_new), __func__)
#define commit_creds_in_task_transition(_new) \
	commit_creds_in_task_transition_where((_new), __func__)
#define commit_creds_setid(_new) commit_creds_setid_where((_new), __func__)
#define abort_creds_setid(_new) abort_creds_setid_where((_new), __func__)
...
#define cred_guard_task_begin_exec_transition(_task) \
	auth_guard_task_begin_exec_transition_where((_task), __func__)
...
/* Restore the prior subjective credential and drop the transient override. */
static inline void revert_creds_and_put_override(const struct cred *cred)
{
	if (cred)
		put_cred(revert_creds(cred));
}

DEFINE_CLASS(override_creds_guarded, const struct cred *,
	     revert_creds_and_put_override(_T),
	     override_creds(override),
	     const struct cred *override)
DEFINE_CLASS_IS_GUARD(override_creds_guarded);

#define scoped_with_creds(_override) \
	scoped_guard(override_creds_guarded, _override)
