...
({                                                                        \
	...
	if (__ag_result == AUTH_GUARD_MUTATION_APPLIED)                       \
		_put(__ag_old);                                                 \
	else if (__ag_result != AUTH_GUARD_MUTATION_QUARANTINED)              \
		_put(__ag_new);                                                 \
	__ag_result;                                                          \
})
...
/* A staged setns nsproxy is private, so its process-context installer can wait. */
#define __auth_guard_nsproxy_install(_operation)                         \
({                                                                      \
	enum auth_guard_mutation_result __ag_result;                       \
	do {                                                               \
		__ag_result = (_operation);                                  \
		if (__ag_result != AUTH_GUARD_MUTATION_BUSY)                 \
			break;                                                \
		cpu_relax();                                                \
		cond_resched();                                             \
	} while (1);                                                       \
	__ag_result == AUTH_GUARD_MUTATION_APPLIED ? 0 : -EACCES;          \
})

#define auth_guard_nsproxy_install_owned(_nsproxy, _member, _new, _get, _put) \
	__auth_guard_nsproxy_install(                                        \
		auth_guard_nsproxy_replace_owned(                              \
			(_nsproxy), _member, (_new), _get, _put))

#define auth_guard_nsproxy_install_time_owned(_nsproxy, _new, _get, _put) \
	__auth_guard_nsproxy_install(                                        \
		auth_guard_nsproxy_replace_time_owned(                         \
 ...
 * only if the complete task/nsproxy snapshot remains valid. Preserve BUSY so
 * procfs can retry after dropping exec_update_lock, not inside this getter.
 */
...
#define consume_pending_child_ns_request(_task) \
	consume_pending_child_ns_request_where((_task), __func__)
#define consume_pending_child_ns_request_in_transition( \
		_task, _payloads)                           \
	consume_pending_child_ns_request_in_transition_where( \
		(_task), (_payloads), __func__)
