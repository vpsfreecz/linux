...
static int prepare_nsset(unsigned flags, struct nsset *nsset)
{
	...
	nsset->flags = flags;
	if (!auth_guard_nsproxy_check_wait(me->nsproxy))
		return -EACCES;
...
static int commit_nsset(struct nsset *nsset)
{
	...
	if (!auth_guard_nsproxy_check_wait(nsset->nsproxy))
		return -EACCES;

	/* Namespace-only setns can wait for legitimate remote readers. */
	if (!(flags & CLONE_NEWUSER ?
	      cred_guard_task_begin_consuming_transition(me) :
	      auth_guard_task_begin_transition_wait(me)))
		return -EACCES;
