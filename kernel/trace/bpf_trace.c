					...
					struct bpf_insn_access_aux *info)
{
	if (bpf_token_is_container(prog->aux->token)) {
		if (info && info->log)
			bpf_log(info->log,
				"container tracing rejects raw pt_regs access; typed kprobe args are not implemented yet\n");
		return false;
	}
				    ...
				    struct bpf_insn_access_aux *info)
{
	...
	if (bpf_token_is_container(prog->aux->token)) {
		...
		if (info && info->log)
			bpf_log(info->log,
				"container tracing rejects raw perf regs/addr context access; typed perf provenance is not implemented yet\n");
		return false;
	}
