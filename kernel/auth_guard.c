...
static u64 auth_guard_transition_hash(
	...
	const struct auth_guard_transition *transition,
	enum auth_guard_transition_anchor anchor,
	const struct auth_guard_stamp *stamp, u64 opener, u32 flags,
	u64 transition_nonce, u64 restore_state, u64 expected_state, u32 depth)
{
	...
	const struct auth_guard_transition *transition)
{
	return auth_guard_transition_anchor_load(transition) ==
		...
		(!transition->flags || !READ_ONCE(*transition->flags)) &&
		(!transition->opener || !READ_ONCE(*transition->opener)) &&
	...
	const struct auth_guard_transition *transition)
{
	...
	if (transition->opener)
		WRITE_ONCE(*transition->opener, 0);
	if (transition->flags)
		WRITE_ONCE(*transition->flags, 0);
	if (transition->anchor)
...
static bool auth_guard_transition_publish(
	...
	const struct auth_guard_transition *transition,
	enum auth_guard_transition_anchor anchor,
	const struct auth_guard_stamp *stamp, u32 flags, u64 restore_state,
	u64 expected_state)
{
	u64 opener;
	u64 nonce;
	...
	if (!auth_guard_transition_anchor_supported(transition, anchor) ||
	    (flags & ~AUTH_GUARD_TRANSITION_ALLOW_SUBJECTIVE_CRED) ||
	...
	opener = transition->opener ? auth_guard_ptr(current) : 0;
	seal = auth_guard_transition_hash(domain, transition, anchor, stamp, opener,
					  flags, nonce,
					  restore_state,
					  expected_state,
	...
	if (transition->flags)
		WRITE_ONCE(*transition->flags, flags);
	if (transition->opener)
		WRITE_ONCE(*transition->opener, opener);
	...
	u64 *restore_state, u64 *expected_state)
{
	...
	transition_opener = transition->opener ?
		READ_ONCE(*transition->opener) : 0;
	transition_flags = transition->flags ? READ_ONCE(*transition->flags) : 0;
	...
	if ((depth != AUTH_GUARD_TRANSITION_OPEN &&
	    ...
	    (transition_flags & ~AUTH_GUARD_TRANSITION_ALLOW_SUBJECTIVE_CRED) ||
	    (transition->opener && !transition_opener) ||
	    auth_guard_transition_anchor_load(transition) != anchor) {
		auth_guard_fail(domain, where, "missing transition",
				transition->object);
		return false;
	}
	if (transition->opener && transition_opener != auth_guard_ptr(current)) {
		auth_guard_fail(domain, where, "foreign transition opener",
				transition->object);
		return false;
	}
	...
	const struct auth_guard_stamp *new_stamp)
{
	...
	nonce = auth_guard_nonce();
	opener = READ_ONCE(*transition->opener);
	flags = transition->flags ? READ_ONCE(*transition->flags) : 0;
	seal = auth_guard_transition_hash(domain, transition, new_anchor, new_stamp,
					  opener, flags, nonce, expected_state,
					  expected_state,
					  AUTH_GUARD_TRANSITION_OPEN);
	...
	const struct auth_guard_stamp *stamp, u64 current_state, u64 expected_state)
{
	...
	nonce = auth_guard_nonce();
	opener = transition->opener ? READ_ONCE(*transition->opener) : 0;
	flags = transition->flags ? READ_ONCE(*transition->flags) : 0;
	seal = auth_guard_transition_hash(domain, transition, anchor, stamp, opener,
					  flags, nonce, authenticated_restore_state,
					  expected_state,
					  AUTH_GUARD_TRANSITION_OPEN);
	WRITE_ONCE(*transition->expected_state, expected_state);
	WRITE_ONCE(*transition->nonce, nonce);
...
enum auth_guard_check_result
auth_guard_task_borrow_subjective_where(struct task_struct *task,
					const char *where)
{
	struct auth_guard_transition transition;
	u32 flags;
	...
	if (!auth_guard_task_transition_enabled() || !task || task != current)
		return AUTH_GUARD_CHECK_BUSY;
	transition = auth_guard_task_transition(task);
	/* Pair with release publication of the complete writer marker. */
	state = smp_load_acquire(transition.depth);
	if (state != AUTH_GUARD_TRANSITION_OPEN &&
	    state != AUTH_GUARD_TRANSITION_QUARANTINED)
		return AUTH_GUARD_CHECK_BUSY;
	if (READ_ONCE(*transition.opener) != auth_guard_ptr(current))
		return AUTH_GUARD_CHECK_BUSY;
	if (state == AUTH_GUARD_TRANSITION_QUARANTINED)
		return AUTH_GUARD_CHECK_INVALID;
	...
	if (flags != AUTH_GUARD_TRANSITION_ALLOW_SUBJECTIVE_CRED) {
		auth_guard_fail(&task_transition_guard, where,
				"subjective credential borrow forbidden", task);
		return AUTH_GUARD_CHECK_INVALID;
	}
	return AUTH_GUARD_CHECK_VALID;
}
	...
	u64 expected_state, const char *where)
{
	...
	return auth_guard_transition_publish(
		...
		flags, restore_state, expected_state);
}
	...
	const struct auth_guard_stamp *stamp, const char *where)
{
	...
	return auth_guard_task_transition_publish_exact_where(
		task, anchor, stamp, 0, state, state, where);
}
...
auth_guard_task_begin_transition_reserved(
	...
	enum auth_guard_transition_anchor anchor, u32 flags)
{
	struct auth_guard_stamp stamp;
	enum auth_guard_check_result result;
	u64 state;
	...
	if (!auth_guard_task_transition_publish_exact_where(
		    task, anchor, &stamp, flags, state, state, where))
		return AUTH_GUARD_CHECK_INVALID;
	return AUTH_GUARD_CHECK_VALID;
}
					   ...
					   const char *where)
{
	...
	return auth_guard_task_begin_transition_reserved(
		...
		AUTH_GUARD_TRANSITION_ANCHOR_CRED, 0);
}
...
static bool auth_guard_cred_transition_valid(struct task_struct *task,
					     const char *where,
					     struct auth_guard_transition *transition_out,
					     struct auth_guard_stamp *stamp_out)
{
	struct auth_guard_transition transition;
	struct auth_guard_stamp stamp;
	enum auth_guard_check_result result;
...
static bool
auth_guard_task_finish_cred_transition_where(struct task_struct *task,
					     const char *where)
{
	struct auth_guard_transition transition;
	struct auth_guard_stamp stamp;

	if (!auth_guard_cred_transition_valid(task, where, &transition, &stamp))
		return false;
	return auth_guard_transition_close(&task_transition_guard, where, &transition,
...
static enum auth_guard_check_result
auth_guard_task_begin_exec_cred_transition_once(struct task_struct *task,
						const char *where)
{
	typeof(auth_guard_task_validate_cred_reservation) *validate =
		auth_guard_task_validate_cred_reservation;
	u32 flags = AUTH_GUARD_TRANSITION_ALLOW_SUBJECTIVE_CRED;

	return auth_guard_task_begin_transition_reserved(task, where,
						 validate,
...
#ifndef CONFIG_AUTH_GUARD
enum auth_guard_check_result
auth_guard_task_begin_transition_try_where(struct task_struct *task,
					   const char *where)
{
...
bool auth_guard_task_begin_transition_where(struct task_struct *task,
					    const char *where)
{
	return auth_guard_task_begin_transition_try_where(task, where) ==
		AUTH_GUARD_CHECK_VALID;
}
...
bool auth_guard_task_begin_exec_transition_where(struct task_struct *task,
						 const char *where)
{
	enum auth_guard_check_result result;
	...
	if (unlikely(!task))
		return false;
	do {
		result = auth_guard_task_begin_exec_cred_transition_once(task, where);
		if (result == AUTH_GUARD_CHECK_BUSY) {
			cpu_relax();
			cond_resched();
		}
	} while (result == AUTH_GUARD_CHECK_BUSY);
...
bool
auth_guard_task_check_in_transition_where(struct task_struct *task,
					  const char *where)
{
	struct auth_guard_transition transition;
	struct auth_guard_stamp stamp;

	return auth_guard_cred_transition_valid(task, where, &transition, &stamp);
}
...
static enum auth_guard_check_result
auth_guard_nsproxy_transition_begin_where(struct nsproxy *nsproxy,
...
struct auth_guard_userns_boundary_digest {
	...
	u64 syslog_ns;
	u64 tracing_ns;
	u64 syslog_ns_is_owner;
	u64 tracing_ns_is_owner;
} __aligned(SIPHASH_ALIGNMENT);
				 ...
				 const struct auth_guard_userns_boundary *right)
{
	return left->syslog_ns == right->syslog_ns &&
		...
		left->syslog_ns_is_owner == right->syslog_ns_is_owner &&
		left->tracing_ns_is_owner == right->tracing_ns_is_owner;
}

static bool
auth_guard_userns_boundary_empty(const struct auth_guard_userns_boundary *boundary)
{
	return !boundary->syslog_ns && !boundary->tracing_ns &&
				...
				const struct auth_guard_userns_boundary *boundary)
{
	struct auth_guard_userns_boundary_digest digest = {
		...
		.syslog_ns_is_owner = boundary->syslog_ns_is_owner,
		.tracing_ns_is_owner = boundary->tracing_ns_is_owner,
	};
					  ...
					  const char *where)
{
	...
	*transaction = (struct auth_guard_nsproxy_transition) {
	...
	transition = auth_guard_nsproxy_transition(nsproxy);
	result = auth_guard_transition_reserve(&nsproxy_authority_guard, where,
					       &transition);
	if (result != AUTH_GUARD_CHECK_VALID)
		return result;
	...
	return auth_guard_transition_publish(
		...
		AUTH_GUARD_TRANSITION_ANCHOR_NONE, &stamp, 0, expected_state,
		expected_state) ? AUTH_GUARD_CHECK_VALID : AUTH_GUARD_CHECK_INVALID;
}
...
({                                                                      \
	...
	enum auth_guard_check_result __ag_begin;                              \
	enum auth_guard_mutation_result __result =                         \
		AUTH_GUARD_MUTATION_REJECTED;                              \
	__ag_begin = auth_guard_nsproxy_transition_begin_where(              \
		(_nsproxy), __ag_transaction, __ag_where);                    \
	if (__ag_begin == AUTH_GUARD_CHECK_BUSY)                             \
		__result = AUTH_GUARD_MUTATION_BUSY;                         \
	else if (__ag_begin == AUTH_GUARD_CHECK_VALID) {                     \
		if (!(_expect)) {                                              \
...
enum auth_guard_check_result
auth_guard_nsproxy_check_status_where(const struct nsproxy *nsproxy,
				      const char *where)
{
	return auth_guard_nsproxy_check_result(nsproxy, where, false);
}

bool auth_guard_nsproxy_check_wait_where(const struct nsproxy *nsproxy,
					 const char *where)
{
	enum auth_guard_check_result result;
...
static enum auth_guard_check_result auth_guard_task_check_endpoints(
	struct task_struct *task, const char *where,
	const struct cred *expected_real_cred, bool check_expected_real_cred,
	...
	u64 *expected_state)
{
	...
	if (!state_hash)
		return AUTH_GUARD_CHECK_INVALID;
	result = auth_guard_task_check_endpoints(task, where, NULL, false, true,
						 false);
	if (result != AUTH_GUARD_CHECK_VALID)
		return result;
...
static enum auth_guard_check_result
auth_guard_task_check_owned_transition(struct task_struct *task,
				       ...
				       const char *where)
{
	...
	/* A task may consume its own sealed writer without reserving a reader. */
	if (task != current)
		return AUTH_GUARD_CHECK_BUSY;
	transition = auth_guard_task_transition(task);
	/* Pair with release publication of the complete writer marker. */
	...
	if ((state != AUTH_GUARD_TRANSITION_OPEN &&
	     state != AUTH_GUARD_TRANSITION_QUARANTINED) ||
	    READ_ONCE(*transition.opener) != auth_guard_ptr(current))
		return AUTH_GUARD_CHECK_BUSY;
	if (state == AUTH_GUARD_TRANSITION_QUARANTINED)
		return AUTH_GUARD_CHECK_INVALID;
	if (!auth_guard_task_validate_transition_result_where(task, where))
		return AUTH_GUARD_CHECK_INVALID;
	if (auth_guard_task_check_endpoints(task, where, expected_real_cred,
					    check_expected_real_cred, true, true) !=
	    AUTH_GUARD_CHECK_VALID) {
		auth_guard_task_transition_quarantine(task);
		return AUTH_GUARD_CHECK_INVALID;
	}
						  ...
						  const char *where)
{
	...
	if (!auth_task_guard_enabled())
		return auth_guard_cred_transition_valid(task, where, &transition,
								 &stamp);
	return auth_guard_task_check_owned_transition(task, NULL, false, where) ==
		AUTH_GUARD_CHECK_VALID;
}
...
static bool auth_guard_task_seal(struct task_struct *task, const char *where)
{
	...
	if (result != AUTH_GUARD_CHECK_VALID)
		goto invalid;
	if (!auth_guard_task_transition_publish_exact_where(
			task, AUTH_GUARD_TRANSITION_ANCHOR_CRED, &cred_stamp, 0,
	...
	state_hash = auth_guard_task_state_hash(
		task, &state, 0, 0, AUTH_GUARD_SEMANTIC_EXPECTATION, where, false);
	if (state_hash != expected_state ||
	    auth_guard_task_check_endpoints(task, where, NULL, false, true,
					    false) !=
		    AUTH_GUARD_CHECK_VALID ||
	    !auth_guard_task_state_read(task, &state, where) ||
	    auth_guard_task_state_hash(
				...
				bool reserved, bool wait)
{
	...
	result = auth_guard_task_check_creds(task, check_expected_real_cred ?
					     expected_real_cred : NULL, where,
					     reserved);
	if (result != AUTH_GUARD_CHECK_VALID)
		return result;
#ifdef CONFIG_CGROUPS
...
bool auth_guard_task_init_check_where(struct task_struct *task, const char *where)
{
	...
	return auth_guard_task_check_endpoints(task, where, NULL, false, false,
					       false) ==
		AUTH_GUARD_CHECK_VALID;
}
			     ...
			     bool retain_reservation)
{
	...
	if (result == AUTH_GUARD_CHECK_BUSY)
		return auth_guard_task_check_owned_transition(task,
			expected_real_cred, check_expected_real_cred, where);
	if (result != AUTH_GUARD_CHECK_VALID)
		return result;
...
bool auth_guard_task_snapshot_end_where(struct task_struct *task, const char *where)
{
	...
	result = auth_guard_task_check_owned_transition(task, NULL, false, where);
	if (result != AUTH_GUARD_CHECK_BUSY)
		return result == AUTH_GUARD_CHECK_VALID;
...
auth_guard_task_begin_transition_once(struct task_struct *task, const char *where)
{
	...
	return auth_guard_task_begin_transition_reserved(
		...
		AUTH_GUARD_TRANSITION_ANCHOR_AUTHORITY, 0);
}

static enum auth_guard_check_result
auth_guard_task_begin_exec_transition_once(struct task_struct *task,
					   const char *where)
{
	typeof(auth_guard_task_validate_reservation) *validate =
		auth_guard_task_validate_reservation;
	u32 flags = AUTH_GUARD_TRANSITION_ALLOW_SUBJECTIVE_CRED;

	if (!auth_task_guard_enabled())
		return auth_guard_task_begin_exec_cred_transition_once(task, where);
	if (!task) {
		auth_guard_fail(&task_authority_guard, where, "missing", task);
		return AUTH_GUARD_CHECK_INVALID;
	}
						 ...
						 const char *where)
{
	...
	if (unlikely(!task))
		return false;
	do {
		result = auth_guard_task_begin_exec_transition_once(task, where);
		if (result == AUTH_GUARD_CHECK_BUSY) {
			cpu_relax();
			cond_resched();
		}
...
retry_reservation:
	...
	if (!auth_guard_task_transition_publish_exact_where(
			task, AUTH_GUARD_TRANSITION_ANCHOR_AUTHORITY, &stamp, 0,
			state, state, where))
		return AUTH_GUARD_TASK_TEARDOWN_OPENED;
				       ...
				       const char *where)
{
	...
	if (auth_guard_task_check_endpoints(task, where, NULL, false, true,
					    true) !=
	    AUTH_GUARD_CHECK_VALID)
		goto endpoint_failed;
