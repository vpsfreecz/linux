...
static void free_user_ns(struct work_struct *work)
{
	...
	do {
		...
		exact &= xchg(&ns->syslog_ns_is_owner, false) ==
			boundary.syslog_ns_is_owner;
#ifdef CONFIG_TRACING_NS
		exact &= xchg(&ns->tracing_ns, NULL) == boundary.tracing_ns;
		exact &= xchg(&ns->tracing_ns_is_owner, false) ==
			boundary.tracing_ns_is_owner;
#endif
		trusted = auth_guard_userns_boundary_destroy_complete(ns, valid,
								      exact);
		if (trusted) {
			if (boundary.syslog_ns_is_owner)
				put_syslog_ns_structural(boundary.syslog_ns);
			else
				put_syslog_ns(boundary.syslog_ns);
#ifdef CONFIG_TRACING_NS
			if (boundary.tracing_ns_is_owner)
				put_tracing_ns_structural(boundary.tracing_ns);
			else
...
u32 get_map_highest_id(struct uid_gid_map *map)
{
	...
	/* Pairs with the map publication smp_wmb() in map_write(). */
	smp_rmb();
...
static struct ns_common *userns_get(struct task_struct *task)
{
	...
	cred = get_task_cred_checked_nowait(task);
	if (IS_ERR(cred))
		return PTR_ERR(cred) == -EAGAIN ? ERR_CAST(cred) : NULL;
