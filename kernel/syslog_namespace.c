			     ...
			     unsigned long new_log_buf_len)
{
	...
	prb = kzalloc(sizeof(struct printk_ringbuffer), GFP_KERNEL);
	if (unlikely(!prb))
		goto fail_no_mem;

	log_buf = kzalloc(new_log_buf_len, GFP_KERNEL);
	if (unlikely(!log_buf))
		goto fail_free_prb;
	...
	descs = kmalloc(descs_size, GFP_KERNEL);
	if (unlikely(!descs))
		goto fail_free_log_buf;

	infos_size = descs_count * sizeof(struct printk_info);
	infos = kmalloc(infos_size, GFP_KERNEL);
	if (unlikely(!infos))
...
{
	kfree(ns->prb->desc_ring.descs);
	kfree(ns->prb->desc_ring.infos);
	kfree(ns->prb);
	kfree(ns->log_buf);
}
