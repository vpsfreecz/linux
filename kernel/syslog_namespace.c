...
{
	struct user_namespace *ucounts_ns = user_ns;

	/* Avoid userns -> syslog ns -> ucounts -> same userns cycles. */
	if (ucounts_ns != &init_user_ns)
		ucounts_ns = ucounts_ns->parent;

	return inc_ucount(ucounts_ns, current_euid(),
...
void syslog_ns_wake_all_waiters(void)
{
	...
	rcu_read_lock();
	for (;;) {
		common = ns_tree_adjoined_rcu(ns, false);
		if (IS_ERR(common))
			break;
		ns = to_syslog_ns(common);
...
void free_syslog_ns(struct syslog_namespace *ns)
{
	...
	/* No external references remain, so drain the final deferred wake. */
	syslog_ns_wake_work_sync(ns);
 ...
 * has init_user_ns as its capability owner. Convert the new user namespace's
 * inherited external reference into its structural ownership reference and
 * move every other external-reference pin to the new capability owner.
 *
 * The owner lock keeps the namespace reference count and its corresponding
 * user-namespace pins paired across the one permitted ownership transition.
 * The old owner is always init_user_ns, so readers which observed it before
 * the transition remain valid while new readers use the container userns.
 * Callers invoke this only after clone/unshare namespace preparation has no
 * remaining failure path, so an aborted operation cannot strand an unpublished
 * user namespace as the owner of a still-live syslog namespace.
 */
void syslog_ns_claim_user_ns(struct user_namespace *user_ns)
{
	...
	struct syslog_namespace *ns = user_ns->syslog_ns;
	struct user_namespace *old_user_ns = NULL;
	unsigned int refs = 0;
	unsigned long flags;
	unsigned int i;

	if (!ns || ns == &init_syslog_ns || user_ns == &init_user_ns)
		return;

	auth_guard_userns_boundary_read(user_ns, &old_boundary);
	AUTH_GUARD_FAIL_STOP_UNLESS(
		auth_guard_userns_boundary_begin_transition_where(
			user_ns, &old_boundary, __func__));

	spin_lock_irqsave(&ns->owner_lock, flags);
	if (ns->user_ns == &init_user_ns) {
		refs = refcount_read(&ns->ns.__ns_ref);
		if (WARN_ON_ONCE(!refs || old_boundary.syslog_ns != ns ||
				 old_boundary.syslog_ns_is_owner))
			goto unlock;

		/* The inherited reference becomes structural and does not self-pin. */
		for (i = 1; i < refs; i++)
			get_user_ns(user_ns);

		old_user_ns = ns->user_ns;
		WRITE_ONCE(ns->user_ns, user_ns);
		WRITE_ONCE(user_ns->syslog_ns_is_owner, true);
	}
unlock:
	...
	if (old_user_ns) {
		...
		proposed.syslog_ns_is_owner = true;
		AUTH_GUARD_FAIL_STOP_UNLESS(
			auth_guard_userns_boundary_finish_transition_where(
				user_ns, &proposed, __func__));
	} else {
		AUTH_GUARD_FAIL_STOP_UNLESS(
			auth_guard_userns_boundary_abort_transition_where(
				user_ns, &old_boundary, __func__));
	}

	/* Drop the old owner pin paired with every existing external reference. */
	if (old_user_ns)
		for (i = 0; i < refs; i++)
...
clone_syslog_ns(struct user_namespace *user_ns,
