...
static int cgroup_auth_guard_errno(enum auth_guard_check_result result)
{
...
css_set_move_auth_check_where(struct task_struct *task,
...
css_set_move_task_auth_guard_check_where(struct task_struct *task,
					 ...
					 const char *where)
{
	return css_set_move_auth_check_where(task, from_cset, to_cset, mode,
					  where) == AUTH_GUARD_CHECK_VALID;
}

#define css_set_move_task_auth_guard_check(_task, _from, _to, _mode) \
	css_set_move_task_auth_guard_check_where((_task), (_from), (_to), \
						 (_mode), __func__)
#define css_set_move_auth_check(_task, _from, _to, _mode) \
	css_set_move_auth_check_where((_task), (_from), (_to), (_mode), __func__)

static enum auth_guard_check_result
css_set_move_auth_begin_where(struct task_struct *task,
			      struct css_set *from_cset,
			      struct css_set *to_cset, const char *where)
{
	enum auth_guard_check_result result;

	result = auth_guard_task_begin_transition_try_where(task, where);
	if (result != AUTH_GUARD_CHECK_VALID)
		return result;
...
#define css_set_move_auth_begin(_task, _from, _to) \
	css_set_move_auth_begin_where((_task), (_from), (_to), __func__)

static int css_set_move_task(struct task_struct *task,
			     ...
			     enum css_set_move_auth_guard_mode auth_guard_mode)
{
	...
	if (auth_guard_mode == CSS_SET_MOVE_AUTH_GUARD_EXIT) {
		...
		if (!preflight_ok ||
		   ...
		   CSS_SET_MOVE_AUTH_GUARD_RUNTIME_PRECHECKED) {
		auth_result = css_set_move_auth_check(task, from_cset, to_cset,
						      auth_guard_mode);
		if (auth_result != AUTH_GUARD_CHECK_VALID)
			return cgroup_auth_guard_errno(auth_result);
	}
	switch (auth_guard_mode) {
	case CSS_SET_MOVE_AUTH_GUARD_RUNTIME:
		auth_result = css_set_move_auth_begin(task, from_cset, to_cset);
		if (auth_result != AUTH_GUARD_CHECK_VALID)
			return cgroup_auth_guard_errno(auth_result);
		auth_guarded = true;
		break;
...
static enum auth_guard_check_result
cgroup_migrate_auth_guard_preflight(struct cgroup_taskset *tset)
{
	struct task_struct *task, *tmp_task;
	struct css_set *cset;
	...
	list_for_each_entry(cset, &tset->src_csets, mg_node) {
		list_for_each_entry_safe(task, tmp_task, &cset->mg_tasks, cg_list) {
			...
			result = css_set_move_auth_check(task, from_cset,
							 to_cset,
							 CSS_SET_MOVE_AUTH_GUARD_RUNTIME);
			if (result != AUTH_GUARD_CHECK_VALID)
				return result;
 ...
 * becomes noop if @task doesn't need to be migrated.  Returns %-EAGAIN if
 * AUTH_GUARD finds a transient task writer and %-EACCES for invalid state.
 * @task's
 ...
 */
static int cgroup_migrate_add_task(struct task_struct *task,
				   struct cgroup_mgctx *mgctx)
{
	...
	if (result != AUTH_GUARD_CHECK_VALID)
		return cgroup_auth_guard_errno(result);
...
static int cgroup_migrate_execute(struct cgroup_mgctx *mgctx)
{
	...
	if (tset->nr_tasks) {
		spin_lock_irq(&css_set_lock);
		auth_result = cgroup_migrate_auth_guard_preflight(tset);
		if (auth_result != AUTH_GUARD_CHECK_VALID) {
			spin_unlock_irq(&css_set_lock);
			ret = cgroup_auth_guard_errno(auth_result);
	...
	list_for_each_entry(cset, &tset->src_csets, mg_node) {
		list_for_each_entry_safe(task, tmp_task, &cset->mg_tasks, cg_list) {
			...
			auth_result = css_set_move_auth_check(task, from_cset,
							      to_cset,
							      CSS_SET_MOVE_AUTH_GUARD_RUNTIME);
			if (auth_result == AUTH_GUARD_CHECK_VALID)
				auth_result = css_set_move_auth_begin(task, from_cset,
								      to_cset);
			if (auth_result != AUTH_GUARD_CHECK_VALID) {
	...
	list_for_each_entry(cset, &tset->src_csets, mg_node) {
		list_for_each_entry_safe(task, tmp_task, &cset->mg_tasks, cg_list) {
			...
			get_css_set(to_cset);
			to_cset->nr_tasks++;
			move_ret = css_set_move_task(task, from_cset, to_cset, true,
						     CSS_SET_MOVE_AUTH_GUARD_RUNTIME_PRECHECKED);
			WARN_ON_ONCE(move_ret);
			from_cset->nr_tasks--;
			/*
		   ...
		   struct cgroup_mgctx *mgctx)
{
	...
	task = leader;
	do {
		ret = cgroup_migrate_add_task(task, mgctx);
		if (ret)
			break;
		if (!threadgroup)
			break;
		       ...
		       bool threadgroup)
{
	...
	do {
		...
		result = auth_guard_task_check_status(task);
		if (result != AUTH_GUARD_CHECK_VALID) {
			ret = cgroup_auth_guard_errno(result);
			break;
		}
...
retry:
	...
	spin_lock_irq(&css_set_lock);
	auth_result = auth_guard_task_check_status(task);
	if (auth_result != AUTH_GUARD_CHECK_VALID) {
		spin_unlock_irq(&css_set_lock);
		ret = cgroup_auth_guard_errno(auth_result);
		goto out_finish;
