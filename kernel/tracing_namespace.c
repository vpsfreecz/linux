...
struct tracing_namespace init_tracing_ns = {
	...
	.pid_ns_id = 0,
	.syslog_ns_id = 0,
	.pid_ns_inum = PID_NS_INIT_INO,
	.syslog_ns_inum = PROC_SYSLOG_INIT_INO,
	.parent = NULL,
	.ns = {
			     ...
			     int res)
{
	...
	audit_log_format(ab,
			 "op=tracing_ns_%s tracing_ns=%u parent=%u user_ns=%u",
			 op, ns ? ns->ns.inum : 0,
			 parent ? parent->ns.inum : 0,
			 user_ns ? user_ns->ns.inum : 0);
	audit_log_format(ab, " pid_ns=%u syslog_ns=%u res=%d",
			 pid_ns_inum, syslog_ns_inum, res);
	audit_log_end(ab);
}
			       ...
			       const struct pid_namespace *pid_ns)
{
	...
	return tracing_ns_pid_contains(ns, pid_ns);
}
EXPORT_SYMBOL_GPL(tracing_ns_matches_pid_ns);
...
EXPORT_SYMBOL_GPL(tracing_ns_matches_user_ns);
...
void free_tracing_ns(struct tracing_namespace *ns)
{
	...
	pr_notice("tracing_ns: destroy ns=%u user=%u pid=%u syslog=%u\n",
		  ...
		  ns->pid_ns_inum,
		  ns->syslog_ns_inum);
	tracing_ns_audit("destroy", ns, ns->parent, ns->user_ns,
			 ns->pid_ns_inum, ns->syslog_ns_inum, 1);
				       ...
				       const struct tracing_namespace *source_ns)
{
	...
	pr_notice("tracing_ns: reject userns setns source=%u target_user=%u target_tracing=%u\n",
		  source_ns->ns.inum,
		  user_ns->ns.inum,
		  target_ns->ns.inum);
	tracing_ns_audit("reject_userns_setns", target_ns, source_ns, user_ns,
			 0, 0, -EPERM);
	return -EPERM;
}
...
{
	return tracing_ns_check_userns_setns_from(user_ns,
						    current_tracing_ns());
}
				      ...
				      const struct tracing_namespace *source_ns)
{
	...
	if (source_ns == &init_tracing_ns) {
		if (target_ns == source_ns)
			return 0;
	} else if (tracing_ns_pid_contains(source_ns, pid_ns)) {
		return 0;
	}

	pr_notice("tracing_ns: reject pidns setns source=%u target_pid=%u target_tracing=%u\n",
		  source_ns->ns.inum,
		  pid_ns->ns.inum,
		  target_ns->ns.inum);
	tracing_ns_audit("reject_pidns_setns", target_ns, source_ns,
			 pid_ns->user_ns, pid_ns->ns.inum, 0, -EPERM);
	return -EPERM;
}
...
{
	return tracing_ns_check_pidns_setns_from(pid_ns,
						   current_tracing_ns());
}
					 ...
					 const struct tracing_namespace *source_ns)
{
	...
	if (source_ns == &init_tracing_ns) {
		if (target_ns == source_ns)
			return 0;
	} else if (tracing_ns_syslog_contains(source_ns, syslog_ns)) {
		return 0;
	}

	pr_notice("tracing_ns: reject syslogns setns source=%u target_syslog=%u target_tracing=%u\n",
		  source_ns->ns.inum,
		  syslog_ns->ns.inum,
		  target_ns->ns.inum);
	tracing_ns_audit("reject_syslogns_setns", target_ns, source_ns,
			 syslog_ns->user_ns, 0, syslog_ns->ns.inum, -EPERM);
	return -EPERM;
}
...
{
	return tracing_ns_check_syslogns_setns_from(syslog_ns,
						    current_tracing_ns());
}
						  ...
						  struct tracing_namespace *old_ns)
{
	...
	ns->pid_ns_inum = pid_ns->ns.inum;
	ns->syslog_ns_inum = syslog_ns->ns.inum;
	...
	pr_notice("tracing_ns: create ns=%u parent=%u user=%u pid=%llu syslog=%llu\n",
		  ...
		  ns->pid_ns_id,
		  ns->syslog_ns_id);
	tracing_ns_audit("create", ns, old_ns, user_ns, pid_ns->ns.inum,
			 syslog_ns->ns.inum, 1);
					  ...
					  struct tracing_namespace *old_ns)
{
	...
	if (old_ns != &init_tracing_ns) {
		...
		tracing_ns_audit("reject_nested_create", old_ns, old_ns->parent,
				 user_ns, pid_ns->ns.inum,
				 syslog_ns->ns.inum, -EPERM);
...
static int tracingns_install(struct nsset *nsset, struct ns_common *new)
{
	...
	tracing_ns_audit("reject_direct_setns", ns, nsproxy->tracing_ns,
			 ns->user_ns, ns->pid_ns_inum, ns->syslog_ns_inum,
			 -EPERM);
	return -EPERM;
}
