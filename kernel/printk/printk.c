				...
				size_t text_len, struct syslog_namespace *ns)
{
	...
	if (cont.len) {
		if (cont.owner == current && (lflags & LOG_CONT)) {
			if (cont_add(facility, level, lflags,
					text, text_len, ns))
				return text_len;
		}
...
asmlinkage int ns_printk_emit(struct syslog_namespace *ns, int facility,
				...
				const char *fmt, ...)
{
	...
	va_start(args, fmt);
	r = ns_vprintk_emit(ns, facility, level, dict, dictlen, fmt, args);
	va_end(args);
