				       ...
				       bool *handover)
{
	...
	if (!prb_read_valid(ns->prb, seq, &r))
		return false;
...
void register_console(struct console *newcon)
{
	...
	if (newcon->flags & CON_PRINTBUFFER) {
		...
		mutex_lock(&ns->syslog_lock);
		write_console_seq(newcon, ns->syslog_seq, false);
		mutex_unlock(&ns->syslog_lock);
	} else {
		/* Begin with next message. */
		write_console_seq(newcon, prb_next_seq(ns->prb), false);
	}
...
static bool __pr_flush(struct console *con, int timeout_ms, bool reset_on_progress)
{
	...
	seq = prb_next_seq(init_syslog_ns.prb);
...
static bool printer_should_wake(struct console *con, u64 seq)
{
	...
	return prb_read_valid(init_syslog_ns.prb, seq, NULL);
}
...
static int printk_kthread_func(void *data)
{
	...
	for (;;) {
		 ...
		 * This pairs with __wake_up_klogd:A.
		 */
		error = wait_event_interruptible(init_syslog_ns.log_wait,
...
void defer_console_output(void)
{
	...
	__wake_up_klogd(&init_syslog_ns, val);
}
