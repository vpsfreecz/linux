				 ...
				 const struct cred *expected, const char *where)
{
	...
	if (result == AUTH_GUARD_CHECK_BUSY && task == current &&
	    auth_guard_task_transition_open_where(mutable_task, where))
		return cred_guard_check_task_cred_reserved_where(task, expected, where);
				   ...
				   const char *where)
{
	...
	return auth_guard_task_transition_cred_published_where(task,
		...
		transition->outer_transition || transition->coordinator_borrowed,
		&transition->coordinator_open, where);
...
int cred_guard_preflight_commit_creds_where(const struct cred *new, const char *where)
{
	...
	result = cred_guard_check_task_cred_where(current, old, where);
	if (result == AUTH_GUARD_CHECK_BUSY)
		return -EAGAIN;
	if (result != AUTH_GUARD_CHECK_VALID)
		return -EACCES;
	...
	if (!cred_guard_verify_commit_source(new, old, where))
		return -EINVAL;
	result = auth_guard_task_check_status_where(current, where);
	if (result == AUTH_GUARD_CHECK_BUSY)
		return -EAGAIN;
	if (result != AUTH_GUARD_CHECK_VALID)
		return -EACCES;
...
static bool
cred_guard_begin_subjective_cred_coordinator(bool consuming,
					     bool *borrowed,
					     const char *where)
{
	enum auth_guard_check_result result;

	result = auth_guard_task_borrow_subjective_where(current, where);
	if (result == AUTH_GUARD_CHECK_VALID) {
		*borrowed = true;
		return true;
	}
	if (result != AUTH_GUARD_CHECK_BUSY)
		return false;
	if (!(consuming ?
	      cred_guard_task_begin_consuming_transition_where(current, where) :
	      auth_guard_task_begin_transition_wait_where(current, where)))
		return false;
	*borrowed = false;
	return true;
}
					    ...
					    const char *where)
{
	...
	if (!cred_guard_verify_committed_cred_where(cred, where))
		return false;
	if (!cred_guard_begin_subjective_cred_coordinator(false, &borrowed, where))
		return false;
	...
	transition->coordinator_borrowed = borrowed;
	return true;
}
					     ...
					     const char *where)
{
	if (!cred_guard_task_publish_transition(transition, where))
		return false;
	if (!transition->coordinator_borrowed && transition->coordinator_open &&
...
/**
 * get_task_cred_unpublished_checked_where - Get fork-constructor credentials
 * @task: private unpublished child task
 * @where: guard diagnostic label supplied by the constructor boundary
 *
 * Pin the objective credentials of a private fork child before its first task
 * seal. Both sides of the capture must retain the exact unpublished lifecycle,
 * complete constructor endpoints, and sealed credential attachment.
 *
 * Return: Pinned credentials, or an error pointer when validation fails.
 */
const struct cred *
get_task_cred_unpublished_checked_where(struct task_struct *task,
					const char *where)
{
	const struct cred *cred;

	if (!task || !auth_guard_task_unpublished_where(task, where) ||
	    !auth_guard_task_init_check_where(task, where))
		return ERR_PTR(-EACCES);
	...
	if (rcu_access_pointer(task->real_cred) != cred ||
	    cred_guard_check_task_cred_where(task, cred, where) !=
		    AUTH_GUARD_CHECK_VALID ||
	    !auth_guard_task_unpublished_where(task, where) ||
	    !auth_guard_task_init_check_where(task, where) ||
	    rcu_access_pointer(task->real_cred) != cred ||
	    !cred_guard_verify_cred(cred, where)) {
		put_cred(cred);
		return ERR_PTR(-EACCES);
	}
...
/**
 * prepare_creds_setid_where - Start a serialized set-ID credential update
 * @where: Guard diagnostic and provenance label for this transition
 *
 * Set-ID syscalls enter here before taking any lock needed by a remote task
 * writer. Keep the exclusive task transition through policy checks and either
 * commit_creds_setid_where() or abort_creds_setid_where().
 *
 * Return: Prepared credentials, or an error pointer.
 */
struct cred *prepare_creds_setid_where(const char *where)
{
	struct cred *new;
						 ...
						 bool finish_task_transition)
{
	 ...
	 * credential pointers (dumpability, pdeath_signal, keyrings, and
	 * ucounts). A late guard failure is an invariant
 ...
 * the transition.
 */
int commit_creds_in_task_transition_where(struct cred *new, const char *where)
{
	if (!where)
		where = __func__;

	return commit_creds_apply_in_task_transition(new, where, false);
}
...
int commit_creds_where(struct cred *new, const char *where)
{
	...
	ret = commit_creds_apply_in_task_transition(new, where, true);
	if (ret)
...
/**
 * commit_creds_setid_where - Complete a serialized set-ID credential update
 * @new: Prepared credentials returned by prepare_creds_setid_where()
 * @where: Guard diagnostic and provenance label for this transition
 *
 * The set-ID transaction is already open, so no remote task writer can enter
 * between policy validation and credential publication.
 *
 * Return: 0 on success, or a negative error code before publication.
 */
int commit_creds_setid_where(struct cred *new, const char *where)
{
	int ret;
...
void abort_creds_setid_where(struct cred *new, const char *where)
{
	if (!where)
		where = __func__;
	abort_creds(new);
...
const struct cred *override_creds_from_prepared(struct cred *override_cred)
{
	...
	if (!cred_guard_validate_prepared_consuming(override_cred, where))
		return NULL;
	if (!cred_guard_begin_subjective_cred_coordinator(true, &borrowed, where)) {
