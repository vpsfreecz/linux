...
static struct task_struct *dup_task_struct(struct task_struct *orig, int node)
{
	...
	/* Pending namespace request payloads remain owned by @orig. */
...
int ksys_unshare(unsigned long unshare_flags)
{
	...
	if (new_cred || new_nsproxy || new_fd) {
		/*
		 * Namespace-only unshare can race a legitimate remote authority
		 * reader, such as LXC opening its child's network namespace while
		 * the child creates a cgroup namespace.  No credential authority is
		 * consumed on this path, so wait for that reader to drain.  Keep the
		 * credential-changing path non-waiting because full authority readers
		 * can span external locks needed by a consuming credential writer.
		 */
		auth_guarded = new_cred ?
			cred_guard_task_begin_consuming_transition(current) :
			auth_guard_task_begin_transition_wait(current);
		if (!auth_guarded) {
			err = -EACCES;
			goto bad_unshare_cleanup_nsproxy;
		}
	...
	if (new_cred) {
		err = commit_creds_in_task_transition(new_cred);
		new_cred = NULL;
		if (err)
			goto bad_unshare_abort_transition;
		syslog_ns_claim_user_ns(current_user_ns());
	}
	...
	if (new_fs || new_fd || do_sysvsem || new_nsproxy) {
		...
		if (new_nsproxy) {
			...
			consume_pending_child_ns_request_in_transition(
				current, &pending_payloads);
		}
...
static int unshare_files_where(bool task_transition_open)
{
	...
	if (task_transition_open ?
	    !auth_guard_task_transition_open(task) :
	    !auth_guard_task_check(task))
		return -EACCES;
	...
	if (task_transition_open) {
		task_lock(task);
		mutation = auth_guard_task_replace_files_in_transition(task, copy, &old);
		task_unlock(task);
	} else {
...
{
	return unshare_files_where(false);
}

int unshare_files_in_task_transition(void)
{
	return unshare_files_where(true);
}
