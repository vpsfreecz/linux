...
static bool vpsa_segment_matches(const struct vpsa_kernfs_filter_rule_segment *segment,
				    ...
				    u16 depth)
{
	...
	for (i = 0; i < rule->depth; i++) {
		...
		if (i >= depth)
			return false;
		if (!vpsa_segment_matches(seg, segments[i], segment_lens[i]))
			return false;
	}
...
EXPORT_SYMBOL_GPL(vpsa_kernfs_filter_path_unavailable);
...
{
	unsigned long token = vpsa_kernfs_filter_visibility_token_current();

	vpsa_kernfs_filter_dentry_set_visibility_token_value(dentry, token);
}

static void vpsa_filter_record_replace(int ret,
				       const struct vpsa_kernfs_filter_parse_error *perr)
{
	...
	if (!ret) {
		...
		if (perr && perr->msg[0])
			...
			snprintf(vpsa_kernfs_filter_last_error,
				 sizeof(vpsa_kernfs_filter_last_error),
...
static int vpsa_kernfs_filter_replace_policy_text(const char *text, size_t len)
{
	...
	if (ret) {
		vpsa_filter_record_replace(ret, &perr);
		return ret;
	}
	...
	if (ret) {
		...
		vpsa_filter_record_replace(ret, NULL);
		return ret;
	}

	vpsa_filter_record_replace(0, NULL);
	return 0;
}
				       ...
				       loff_t *ppos)
{
	...
	if (!state || atomic_xchg(&state->attempted, 1))
		return -EINVAL;
	if (*ppos != 0)
		return -EINVAL;
	if (!count)
		return -EINVAL;
	if (count > VPSA_KERNFS_FILTER_POLICY_MAX_BYTES)
		return -E2BIG;
	...
	ret = vpsa_kernfs_filter_replace_policy_text(buf, count);
	kfree(buf);
	if (ret)
		return ret;
...
static int vpsa_kernfs_filter_replace_release(struct inode *inode,
					      struct file *file)
{
