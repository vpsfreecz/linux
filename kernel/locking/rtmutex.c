					    ...
					    struct rt_mutex_base *lock)
{
	 ...
	 */
	preempt_disable();
	clear_task_blocked_on_rtmutex(waiter->task, lock);
				  ...
				  struct rt_mutex_waiter *waiter)
{
	...
	scoped_guard(raw_spinlock, &waiter_task->pi_lock) {
		...
		raw_spin_lock(&waiter_task->blocked_lock);
		__clear_task_blocked_on_rtmutex(waiter_task, lock);
		raw_spin_unlock(&waiter_task->blocked_lock);
	}
