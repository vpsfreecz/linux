...
static unsigned int
percpu_rwsem_read_count_sum(unsigned int __percpu *read_count)
{
	unsigned int sum = 0;
	int cpu;

	for_each_possible_cpu(cpu)
		sum += *per_cpu_ptr(read_count, cpu);
...
static bool
percpu_rwsem_proxy_reader_lock_maybe(struct percpu_rwsem_proxy_reader_slot *slot,
...
{
	/* Publish slot updates before clearing the sampling lock bit. */
...
bool percpu_rwsem_proxy_read_current_is_tracked(struct percpu_rw_semaphore *sem)
{
	int i;

	for (i = 0; i < PERCPU_RWSEM_PROXY_READERS; i++) {
		unsigned long value = READ_ONCE(sem->proxy_readers[i].task);
...
EXPORT_SYMBOL_GPL(percpu_rwsem_proxy_read_current_is_tracked);
...
static inline void
percpu_rwsem_clear_waiter_blocked_on(struct task_struct *p,
				     struct percpu_rw_semaphore *sem)
{
	if (sched_proxy_exec())
		clear_task_blocked_on_percpu_rwsem(p, sem);
}
			 ...
			 enum percpu_rwsem_proxy_owner_state *state)
{
	...
	if (writer_waiting_readers) {
		...
		readers_active = percpu_rwsem_read_count_sum(sem->read_count) != 0;
		if (readers_active) {
				      ...
				      void *key)
{
	...
	percpu_rwsem_clear_waiter_blocked_on(p, sem);
...
{
	return percpu_rwsem_read_count_sum(sem->read_count) != 0 &&
...
{
	if (percpu_rwsem_read_count_sum(sem->read_count) != 0)
		return false;
