...
static bool rwsem_proxy_reader_is_tracked(struct rw_semaphore *sem,
					  struct task_struct *task)
{
	int i;
...
static inline bool rwsem_proxy_reader_is_tracked(struct rw_semaphore *sem,
						 struct task_struct *task)
{
	return false;
}
			    ...
			    struct wake_q_head *wake_q)
{
	...
	if (waiter->type == RWSEM_WAITING_FOR_WRITE) {
		if (wake_type == RWSEM_WAKE_ANY) {
			 ...
			 */
			clear_task_blocked_on_rwsem(waiter->task, sem);
	...
	list_for_each_entry_safe(waiter, tmp, &wlist, list) {
		...
		clear_task_blocked_on_rwsem(tsk, sem);
...
static inline int __down_read_trylock(struct rw_semaphore *sem)
{
	...
	if (ret) {
		proxy_reader_nested = rwsem_proxy_reader_is_tracked(sem, current);
		proxy_reader = rwsem_proxy_reader_try_acquire(sem);
	}
	preempt_enable();
...
static inline void __up_read(struct rw_semaphore *sem)
{
	...
	/* Never leave a stale representative after the reader count reaches 0. */
