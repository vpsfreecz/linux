...
static int meminfo_proc_show(struct seq_file *m, void *v)
{
	...
	if (memcg) {
		...
		cached = vpsadminos_saturating_sub(cached, swapcache);
		...
		i.freeram = vpsadminos_saturating_sub(totalram, memusage);
		...
		available = min(available, totalram);
		committed = 0;
		goto show;
	}
#endif
	...
	cached = vpsadminos_saturating_sub(cached, total_swapcache_pages());
	cached = vpsadminos_saturating_sub(cached, i.bufferram);
...
show:
	...
	show_val_kb(m, "LowTotal:       ",
		    vpsadminos_saturating_sub(i.totalram, i.totalhigh));
	show_val_kb(m, "LowFree:        ",
		    vpsadminos_saturating_sub(i.freeram, i.freehigh));
#endif
