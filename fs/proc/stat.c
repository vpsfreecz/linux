...
static int show_stat(struct seq_file *p, void *v)
{
	...
	if (current->nsproxy && current->nsproxy->cgroup_ns != &init_cgroup_ns) {
		...
		fake = fake_cputime_readout(current, uptime, &user, &system,
					    &cpu_fake_mask);
		if (fake) {
			idle = fake_cputime_idle(uptime, user, system,
						 cpumask_weight(&cpu_fake_mask));
			goto cont_normal;
		}
	}
...
fake_online_cpus:
	for_each_cpu(i, &cpu_fake_mask) {
		...
		idle = fake_cputime_idle(uptime, user, system, 1);
		seq_printf(p, "cpu%d", i);
		seq_put_decimal_ull(p, " ", nsec_to_clock_t(user));
		seq_put_decimal_ull(p, " ", nsec_to_clock_t(nice));
		seq_put_decimal_ull(p, " ", nsec_to_clock_t(system));
